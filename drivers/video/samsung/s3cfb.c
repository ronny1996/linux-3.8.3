...
#if	defined(LCD_BACKLIGHT_CTRL)
static void s3cfb_set_backlight_power_on(void)
{
	uint reg_f_cfg=readl(S3C64XX_GPFCON) & 0xEFFFFFFF | (1<<28);
        writel(reg_f_cfg,S3C64XX_GPFCON);

        uint reg_f_on=readl(S3C64XX_GPFDAT) & 0xFFFFBFFF | (1<<14);
       	// uint reg_f_off=readl(S3C64XX_GPFDAT) & 0xFFFFBFFF;
	//uint reg_f_sta=to?reg_f_on:reg_f_off;
	//writel(reg_f_sta,S3C64XX_GPFDAT);
	writel(reg_f_on,S3C64XX_GPFDAT);
}
#endif
